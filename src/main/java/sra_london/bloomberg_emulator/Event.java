...
public abstract class Event
{
	...
	{	
		Event result;
		if(request.getClass() == sra_london.bloomberg_emulator.IntradayBarRequest.class)
		{
			result = new sra_london.bloomberg_emulator.IntradayBarEvent((sra_london.bloomberg_emulator.IntradayBarRequest)request);
			result._eventType = isLastRequest ? Event.EventType.RESPONSE : Event.EventType.PARTIAL_RESPONSE;
			return result;
		}
		else if(request.getClass() == sra_london.bloomberg_emulator.IntradayTickRequest.class)
		{
			result = new sra_london.bloomberg_emulator.IntradayTickEvent((sra_london.bloomberg_emulator.IntradayTickRequest)request);
			result._eventType = isLastRequest ? Event.EventType.RESPONSE : Event.EventType.PARTIAL_RESPONSE;
			return result;
		}
		else if(request.getClass() == sra_london.bloomberg_emulator.HistoricRequest.class)
		{
			result = new sra_london.bloomberg_emulator.HistoricEvent((sra_london.bloomberg_emulator.HistoricRequest)request);
			result._eventType = isLastRequest ? Event.EventType.RESPONSE : Event.EventType.PARTIAL_RESPONSE;
			return result;			
		}
		else if(request.getClass() == sra_london.bloomberg_emulator.ReferenceRequest.class)
		{
			result = new sra_london.bloomberg_emulator.ReferenceEvent((sra_london.bloomberg_emulator.ReferenceRequest)request);
			result._eventType = isLastRequest ? Event.EventType.RESPONSE : Event.EventType.PARTIAL_RESPONSE;
			return result;
		}
