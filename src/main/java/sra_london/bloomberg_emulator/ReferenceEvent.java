...
import sra_london.bloomberg_emulator.CorrelationID;
...
public class ReferenceEvent extends Event
{
    ...
    private List<Message> generateMessages() throws Exception
    {
        ...
        for(int i = 0; i < rreq.securities().size(); i++)
        {
        	...
        	if(!securities.containsKey(security))
        	{
        		...
        		for(int j = 0; j < rreq.fields().size(); j++)
        		{
                    ...
                    Object value = sra_london.bloomberg_emulator.RandomDataGenerator.referenceDataFromFieldName(field, security, isOption, rreq);
        ...
        sra_london.bloomberg_emulator.ReferenceMessage msg = new sra_london.bloomberg_emulator.ReferenceMessage(corr, securities);
        result.add(msg);
